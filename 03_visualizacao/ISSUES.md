...
> **Status:** Issues das aulas 1 a 6 (incluindo práticas) foram revisados e corrigidos em 2026-06-29. Issues das aulas 7 a 16, práticas 08 e 09-14, e pendências remanescentes das aulas 1 a 6 foram corrigidos em 2026-07-05. Restam apenas 3 sugestões sem ação (informativas ou de decisão estrutural — ver itens marcados como ⏸️).
>
> *Nota: os totais foram recontados em 2026-07-05 a partir dos itens listados neste documento (os totais anteriores estavam subestimados). Os issues transversais não entram na contagem por duplicarem itens por aula.*
...
🟢 **[Sugestão] cell-6 — Imagem do ciclo de vida com URL de fonte externa** — ⏸️ **Sem ação**  
...
> *Sem ação em 2026-07-05: a URL exata do artigo original não é conhecida; adicionar um link chutado seria pior que manter apenas o domínio. Revisitar se a fonte for localizada.*

🟢 **[Sugestão] cell-11 — Links de exemplos externos podem ficar quebrados** — ⏸️ **Sem ação**  
...
> *Sem ação em 2026-07-05: item informativo de manutenção contínua, não há correção pontual a aplicar.*
...
🟡 **[Melhoria] cell-18 — Exemplo de código complexo sem justificativa para o nível da aula** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): exemplo substituído por uma lista simples de pokemons exibida como resultado da célula; texto da cell-17 reescrito de acordo. O conceito demonstrado (resultado aparece abaixo da célula, sem `print`) foi mantido.*
...
🟢 **[Sugestão] cell-10 — Explicação de "overhead no processamento" pode ser mais precisa** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionada nota 📝 na cell-10 esclarecendo que a explicação é uma simplificação pedagógica.*
...
🟡 **[Melhoria] cell-43 — Seaborn introduzido sem instalação/import completo** — ✅ **Corrigido**  
...
> *Verificado em 2026-07-05: a célula já contém `import seaborn as sns` antes da chamada de `sns.countplot()` — o issue já havia sido resolvido em revisão anterior. Nenhuma alteração adicional necessária.*
...
🟢 **[Sugestão] cell-60 — Aviso sobre `iterrows()` sem alternativa prática imediata** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): aviso da cell-60 complementado com os casos em que `iterrows()` ainda é aceitável (conjuntos pequenos, código legado).*
...
🟢 **[Sugestão] Exercício 5 — `sort_values()` não foi ensinado formalmente na Aula 05** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): dica do Exercício 5 agora apresenta a função (`sort_values()` ordena um DataFrame pelos valores de uma coluna) antes de explicar `ascending=False`, que já estava descrito.*
...
🟢 **[Sugestão] Exercício 5 — O método `.plot(kind='bar', x='Generation', stacked=True)` tem `x='Generation'` errado** — ✅ **Corrigido**  
...
> *Verificado em 2026-07-05: o enunciado atual do Exercício 5 não menciona mais `x='Generation'` — usa apenas `.plot(kind='bar', stacked=True)`, consistente com a dica. Já havia sido corrigido em revisão anterior.*
...
🟡 **[Melhoria] cell-2 — "eixo de valores categóricos mas contínuos" — definição contraditória** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): texto corrigido para "eixo de valores contínuos ou discretos ordenados — como datas, gerações ou sequências".*

🟡 **[Melhoria] cell-15 — `stackplot()` não tem legenda exibida** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionado `plt.legend(loc='upper left')` com comentário explicativo antes do `plt.show()`.*

🟢 **[Sugestão] Aula 07 — Ausência de prática dedicada** — ⏸️ **Sem ação**  
...
> *Sem ação em 2026-07-05: criar uma prática nova é decisão estrutural do curso (afeta CONTEUDO.md e PLANO_DE_AULA.md), não uma correção pontual. Pode ser criada com o skill `/nova-aula --pratica` se desejado.*
...
🟡 **[Melhoria] cell-12 — "a distribuição não é uniforme, pois como o valor é numérico ele criou faixas de valores"** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-12 reescrita — explica que o Seaborn interpreta colunas numéricas como escala contínua de cor (gradiente) e que a solução é transformar o valor em texto (categoria).*

🟢 **[Sugestão] cell-24 — Quadrante com valores fixos hardcoded** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): comentário do código dos quadrantes (cell-25) expandido com o exemplo de cálculo dinâmico `limite = max(pokemons['Att'].max(), pokemons['Def'].max()) * 1.05`.*
...
🟢 **[Sugestão] Exercício 4 — Limites de eixo de 0 a 260 não refletem os dados reais** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionada nota 📝 na dica do Exercício 4 explicando que os limites são fixos por simplicidade (maior `HP` = 255) e mostrando como calculá-los dinamicamente.*
...
🟡 **[Melhoria] cell-19 — `groupby` com duas colunas sem necessidade** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): groupby simplificado para `['Number']` apenas, com comentário explicando que é a única coluna necessária para o gráfico.*

🟢 **[Sugestão] cell-19 — `sort_values('Generation Desc', inplace=True)` ordena em ordem lexicográfica, não numérica** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionada nota 📝 na cell-20 explicando a diferença entre ordenação lexicográfica e numérica, com o exemplo de `Gen 10` aparecendo entre `Gen 1` e `Gen 2`.*
...
🔴 **[Crítico] cell-62 — "Esta aula também marca o final do nosso curso de Visualização de Dados com Python!"** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-62 reescrita — agora encerra o módulo de Seaborn/Matplotlib e antecipa o que vem nas próximas aulas (visualizações avançadas, bancos de dados e Plotly).*

🟡 **[Melhoria] cell-38 — `plt.ticklabel_format(style="plain", useLocale=True)` com `useLocale=True`** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionada nota 📝 na cell-37 explicando o que `style="plain"` e `useLocale=True` fazem e avisando que o resultado pode variar conforme o locale do ambiente.*

🟡 **[Melhoria] Aula 10 — O arquivo de dados é carregado via URL do GitHub, mas o CONTEUDO.md diz dataset `worldpop`** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-10 agora apresenta o `world_population.csv` (conteúdo, período coberto e por que ele substitui o `pokemons.csv` nesta aula).*

🟡 **[Melhoria] Passos 1 a 5 — O código completo é repetido em cada passo** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): todas as 11 células de código dos passos receberam um comentário inicial `# >>> O QUE MUDOU NESTE PASSO: ...` descrevendo a novidade em relação ao bloco anterior.*
...
🟡 **[Melhoria] cell-5 — "O nosso problema gira em torno dessa temática" sem mencionar o dataset de pokemons** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionada nota 📝 na cell-5 avisando da troca de dataset (worldcup.json) e indicando que os pokemons voltam no exercício de rede da prática de consolidação.*

🟡 **[Melhoria] cell-14 — "nodos (vértices) e arestas (edges)" — terminologia misturada** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-14 reescrita com terminologia padrão — nodos/vértices (*nodes*) para os elementos, arestas (*edges*) para as conexões; cell-16 corrigida de "nodos e vertices" para "nodos e arestas".*

🟢 **[Sugestão] cell-17 — Parâmetro `linewidths=2` sem relação com `width=2` no options** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionados comentários inline no dict `options` distinguindo `linewidths` (borda dos nodos) de `width` (arestas).*
...
🟡 **[Melhoria] cell-8 — Dataset carregado de URL diferente do padrão** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): comentário na cell-8 esclarece que `all.csv` é o mesmo conjunto de dados do `pokemons.csv`, lido direto da URL do repositório de datasets do curso.*

🟡 **[Melhoria] cell-3 — História do Sankey sem referência específica** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): texto atualizado com o nome completo (Matthew Henry Phineas Riall Sankey), o ano (1898) e o contexto (eficiência energética de máquinas a vapor); typo "durate" também corrigido.*

🟢 **[Sugestão] cell-13 — Gráfico interativo sem instrução para testar a interatividade** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-14 agora descreve o conteúdo dos tooltips (nodo: nome e total; ligação: origem, destino e contagem), útil também para leitura em versão estática.*
...
🟡 **[Melhoria] cell-7 — Instrução de instalação diz que o wordcloud já está no Colab** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): contradição eliminada — cell-7 explica que a instalação é segura em qualquer ambiente, e a cell-8 agora executa `!pip install wordcloud -q` antes dos imports.*

🔴 **[Crítico] cell-14 — `iterrows()` usado para construir dicionário, mas uso desaconselhado em Aula 05** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-14 reescrita usando `lista = dict(zip(df['Name'], df['Att']))`, com comentário relembrando o aviso de performance da Aula 05.*

🟡 **[Melhoria] cell-12 — Dataset carregado da mesma URL alternativa (`all.csv`)** — ✅ **Corrigido**  
Mesmo problema da Aula 12: a aula usa `all.csv` em vez do `pokemons.csv` local, sem explicação da diferença.  
> *Correção aplicada (2026-07-05): mesmo esclarecimento da Aula 12 adicionado no comentário da cell-12.*
...
🔴 **[Crítico] cell-9 — `%reload_ext sql` depende de `jupysql` que não foi instalado** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-9 agora inicia com `!pip install jupysql duckdb duckdb-engine -q`; cell-10 explica por que a instalação é necessária.*

🟡 **[Melhoria] cell-3 — Aviso "pode ser pulada" sem orientação de onde continuar** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): texto agora orienta a seguir direto para a Aula 15 (Gráficos Interativos) caso o aluno prefira pular.*

🟡 **[Melhoria] cell-18 — `table_name` e `file_name` com arquivo `teste.txt` não está no repositório** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): a cell-18 agora cria o `teste.txt` programaticamente (com `open`/`write`) antes de carregá-lo no banco, garantindo que o exemplo funcione em qualquer ambiente.*

🟢 **[Sugestão] cell-30 — Sintaxe `%sql pokemons_agg << SELECT ...` não explicada** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): comentário na célula do `<<` (cell-26) explica que o operador executa a consulta e salva o resultado na variável Python como DataFrame (graças ao `autopandas`).*
...
🔴 **[Crítico] cell-3 — "Esta é nossa última aula" — mas há a Aula 16** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): texto corrigido para "Esta é a nossa penúltima aula". Adicionalmente, a cell-6 recebeu o bloco de instalação do jupysql/duckdb (mesmo problema crítico da Aula 14, que também afetava esta aula).*

🟡 **[Melhoria] cell-15/cell-27 — Problema de escala entre gráfico de linha e área não resolvido** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-27 agora aponta explicitamente que a solução vem na Aula 16, citando `update_layout()`/`update_yaxes()` e o ajuste de faixa do eixo Y como o caminho para eliminar a discrepância.*

🟡 **[Melhoria] cell-8 — DuckDB em memória para transformações simples** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): adicionada nota 📝 na cell-7 esclarecendo que o `GROUP BY` poderia ser feito com Pandas e que o DuckDB é usado apenas como prática da aula anterior.*

🟢 **[Sugestão] cell-35 — `fig.show()` sem `renderer='notebook'`** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-35 atualizada para `fig.show(renderer='notebook')`, padronizando com o restante da aula.*
...
🟡 **[Melhoria] cell-9 — Dataset `ds_salaries.csv` introduzido sem contexto** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): seção "Carregando Dados" (cell-8) agora apresenta o `ds_salaries.csv` e descreve as colunas relevantes (`work_year`, `job_title`, `salary_in_usd`).*

🟡 **[Melhoria] cell-11 — Resultado SQL retorna tuplas, não DataFrame** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): cell-7 agora configura `%config SqlMagic.autopandas = True` (mais `feedback` e `displaycon`), fazendo as queries `%%sql` retornarem DataFrames formatados. A cell-7 também recebeu o bloco de instalação do jupysql/duckdb (mesmo problema crítico da Aula 14, que também afetava esta aula).*

🟢 **[Sugestão] cell-33 — Renomeação de colunas via `labels={}` no `px.bar()` não descrita** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): seção "Ajustando a Tooltip" (cell-32) agora explica o parâmetro `labels` e seu efeito nos eixos, na legenda e na tooltip.*
...
🟡 **[Melhoria] Exercício 8 — Código de instalação do `jupysql` ausente** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): a primeira célula do Exercício 8 agora executa `!pip install jupysql duckdb duckdb-engine -q`; enunciado e dica atualizados para incluir o passo de instalação.*

🟡 **[Melhoria] Exercício 5 — `dropna()` mencionado na dica mas não no enunciado** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): enunciado do Exercício 5 ganhou o passo explícito de remover linhas com `Type 2` nulo via `dropna()` (com o motivo), e `dropna()` foi adicionado às funções úteis.*

🟢 **[Sugestão] Exercício 7 — Dica usa `iterrows()` para construir o dicionário** — ✅ **Corrigido**  
...
> *Correção aplicada (2026-07-05): dica do Exercício 7 atualizada para `dict(zip(pokemons['Name'], pokemons['Def']))`, com referência ao aviso de performance da Aula 05.*
...
- A Aula 07 não tem prática dedicada — os gráficos de linha e área ficam sem exercício específico *(pendente — decisão estrutural)*
- ~~A Aula 04 tem uma seção prometida sobre "bibliotecas externas" que está completamente vazia~~ ✅ Corrigido
- ~~As aulas 11 a 14 usam datasets diferentes (futebol, pokemons via URL, wordcloud, salários) introduzidos abruptamente, quebrando o padrão estabelecido nas aulas anteriores~~ ✅ Mitigado — cada troca de dataset agora é anunciada e explicada no próprio notebook (Aulas 10, 11, 12, 13 e 16)
...
| Aula 02 | ✅ Corrigido — atalhos Ctrl+S/Ctrl+Shift+P; exemplo de código simplificado |
| Aula 03 | ✅ Corrigido — cell-35 convertida para markdown; exemplo de Series; nota sobre simplificação |
| Aula 04 | ✅ Corrigido — seção "Bibliotecas externas" preenchida; import do seaborn verificado |
| Aula 05 | ✅ Corrigido — referência cruzada; contexto de uso do `iterrows()` |
| Prática 05 | ✅ Corrigido — dica do Exercício 1; apresentação do `sort_values()` |
| Aula 06 | ✅ Corrigido — NumPy introduzido; errwidth→err_kws; transição Matplotlib; explicação .plot() |
| Prática 06 | ✅ Corrigido — errwidth→err_kws; enunciado do Ex. 5 sem `x=` incorreto |
| Aula 07 | ✅ Corrigido — definição de linhas/áreas; legenda do stackplot (sem prática dedicada — OK pelo CONTEUDO.md) |
| Aula 08 | ✅ Corrigido — explicação do `hue` numérico; exemplo de limites dinâmicos |
| Prática 08 | ✅ Corrigido — nota sobre limites dinâmicos no Ex. 4 |
| Aula 09 | ✅ Corrigido — groupby simplificado; nota sobre ordenação textual |
| Aula 10 | ✅ Corrigido — encerramento falso removido; dataset apresentado; nota useLocale; comentários por passo |
| Aula 11 | ✅ Corrigido — troca de dataset anunciada; terminologia de grafos padronizada |
| Aula 12 | ✅ Corrigido — `all.csv` esclarecido; história do Sankey completada; tooltips descritos |
| Aula 13 | ✅ Corrigido — instalação sem contradição; `iterrows()` substituído; `all.csv` esclarecido |
| Aula 14 | ✅ Corrigido — `jupysql` instalado; `teste.txt` criado no notebook; operador `<<` explicado |
| Prática 09-14 | ✅ Corrigido — instalação no Ex. 8; `dropna()` no enunciado do Ex. 5; `iterrows()` removido da dica do Ex. 7 |
| Aula 15 | ✅ Corrigido — "penúltima aula"; instalação jupysql; notas DuckDB/escala; renderer padronizado |
| Aula 16 | ✅ Corrigido — dataset `ds_salaries` apresentado; autopandas configurado; `labels` explicado |

### Issues Transversais

🔴 **Afirmação de "última aula" em lugares errados:** ✅ **Corrigido (2026-07-05)** — Aula 10 agora encerra apenas o módulo de Seaborn/Matplotlib e antecipa os próximos módulos; Aula 15 diz "penúltima aula". A Aula 16 permanece como o encerramento real do curso.

🔴 **JupySQL não instalado em Aula 14 e Prática 09-14:** ✅ **Corrigido (2026-07-05)** — adicionado `!pip install jupysql duckdb duckdb-engine -q` na Aula 14, na Prática 09-14 e também nas Aulas 15 e 16 (que usam `%reload_ext sql` e tinham o mesmo problema, embora não tivessem sido listadas originalmente).

🟡 **Uso inconsistente de `errwidth=0` vs `err_kws={"linewidth": 0}`:** ✅ **Corrigido** — verificação por busca global em 2026-07-05 confirmou que não há nenhuma ocorrência remanescente de `errwidth` nos notebooks.

🟡 **Dois datasets "pokemons" diferentes:** ✅ **Corrigido (2026-07-05)** — Aulas 12, 13 e 15 usam `all.csv` via URL; as Aulas 12 e 13 agora têm comentário esclarecendo que é o mesmo conjunto de dados do `pokemons.csv`, lido do repositório de datasets do curso.

🟡 **`iterrows()` desaconselhado na Aula 05 mas usado na Aula 13 e Prática 09-14:** ✅ **Corrigido (2026-07-05)** — ambos os usos substituídos por `dict(zip(...))`, com referência explícita ao aviso da Aula 05; o aviso da Aula 05 também ganhou os casos em que `iterrows()` ainda é aceitável.
